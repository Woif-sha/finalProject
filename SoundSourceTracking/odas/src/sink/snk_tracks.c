    ...
    */
    ...
    void snk_tracks_process_format_text_json(snk_tracks_obj * obj) {
        ...
        for (iTrack = 0; iTrack < obj->nTracks; iTrack++) {
            ...
            sprintf(obj->buffer,"%s        { \"id\": %llu, \"tag\": \"%s\", \"x\": %1.3f, \"y\": %1.3f, \"z\": %1.3f, \"activity\": %1.3f, \"sita\": %1.3f, \"h\": %1.3f }", 
                    obj->buffer,
                    ...
                    atan2(obj->in->tracks->array[iTrack*3+1],obj->in->tracks->array[iTrack*3+0]) * 180 / 3.1415926,
                    acos(obj->in->tracks->array[iTrack*3+2]/sqrt(obj->in->tracks->array[iTrack*3+0]*obj->in->tracks->array[iTrack*3+0]+obj->in->tracks->array[iTrack*3+1]*obj->in->tracks->array[iTrack*3+1]+obj->in->tracks->array[iTrack*3+2]*obj->in->tracks->array[iTrack*3+2])));
        ...
        if(i > 100) {
            ...
            FILE *fp;
            fp = fopen("sst.txt","w");
            int retVal = fwrite(obj->buffer, sizeof(char), obj->bufferSize, fp);
            fclose(fp);
        }
        // fp->close();
        // std::cout << "xxxxx" << obj->buffer << endl;
    }
